...
Planning workflows must run with the highest reasoning level available in the current environment.
...
Before starting any of these workflows, verify the session is configured for strong reasoning. If a specific premium model is unavailable, continue with the best available model and document that constraint in the output.

## 6. Symlink Integrity and Multi-Agent Portability

- **Canonical Source**: Keep reusable artifacts in `ai-specs` as the canonical source. Any agent-specific paths should reference them through symlinks when possible.
