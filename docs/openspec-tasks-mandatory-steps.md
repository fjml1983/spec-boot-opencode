...
- **Step N+3**: E2E Testing with Playwright (CLI or MCP) (MANDATORY if applicable) - **AGENT MUST EXECUTE**
...
### Step N+3: E2E Testing with Playwright (CLI or MCP) (MANDATORY if applicable)

**Agent Responsibility**: The coding agent MUST execute all E2E tests using available Playwright tooling (CLI or MCP). This is NOT optional and cannot be delegated to the user.
...
1. **Prepare Test Environment**:
   ...
   - Ensure Playwright is available in the environment (local CLI or MCP)

2. **Navigate to Application**:
   - Use Playwright automation to open the application URL
   - Wait for page to load completely
   - Take a snapshot to verify initial state

3. **Execute User Workflows**:
    - Use Playwright tooling to interact with the UI (CLI tests or MCP actions):
       - click/navigation actions
       - input/form actions
       - state verification snapshots/assertions
...
- Backend server running (agent must start if needed)
- Playwright available (CLI and/or MCP)
...
## 11. Frontend: E2E Testing with Playwright (CLI or MCP) (MANDATORY if applicable - AGENT MUST EXECUTE)
- [ ] 11.1 Ensure frontend and backend servers are running
- [ ] 11.2 Navigate to application using Playwright automation (CLI or MCP)
- [ ] 11.3 Execute complete user workflow using Playwright automation
...
1. **Execute All Manual Tests**: Never ask the user to run curl commands or E2E tests. The agent must:
   ...
   - Execute all E2E tests using Playwright tooling (CLI or MCP)
