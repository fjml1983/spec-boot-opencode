...
If the native flow does not propagate agent settings and the user expects parity with the main checkout, copy the relevant local settings files from the primary workspace using the same loop as Step 1b **after** the native tool finishes—only when files exist on disk.
...
**Capture the main checkout root before `git worktree add`.** After `cd` into the new worktree, `git rev-parse --show-toplevel` points at the worktree directory, not the checkout where agent settings files usually live.
...
#### Copy local agent configuration (Step 1b only)

After creating the worktree with git (Step 1b), optionally copy local agent settings from the **main checkout** so behavior matches the primary workspace. These files are often untracked.
...
**Symlinks:** If the source tree uses symlinks for agent configuration folders, copying only JSON files does not recreate symlink targets. Either repeat the same symlink layout in the worktree or rely on project-relative paths inside `settings.json`.
...
Agent settings copied (or none found / skipped per harness)
...
### Copying agent settings from the wrong directory
