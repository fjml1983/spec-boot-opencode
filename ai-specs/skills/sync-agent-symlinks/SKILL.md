...
description: Analyze and synchronize agent skill exposure after ai-specs skill changes (additions, removals, renames). Use when skills are added/removed in ai-specs and optional mirror directories must stay aligned through symlinks.
author: LIDR.co
version: 1.0.0
---
...
2. Mirror entries in each detected or user-provided mirror target
...
Expected behavior:
- Add missing symlink in each configured mirror target
- Verify both links resolve to canonical folder

### Scenario B - Skill removed from ai-specs

Expected behavior:
- Remove orphan canonical symlink from each configured mirror target
