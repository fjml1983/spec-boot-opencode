...
Alternative for step 2 (optional helper installer):
...
Your coding agent should automatically load:
...
### Optional Integrations (Jira + Playwright)

This workflow can be enhanced with MCP servers. They are optional and you can skip them entirely, or replace them with equivalent free/local tools.

- **Jira MCP (optional)**: lets the agent read Jira tickets directly. Useful for teams that already use Jira.
- **Playwright MCP (optional)**: lets the agent run browser-based E2E checks.
- **Low-cost default**: use direct ticket text in markdown plus Playwright CLI tests executed locally.

Recommended installation approach:

- Start with no MCP integrations.
- Add MCP only when your team needs it and has the budget/credentials.
- Prefer local, open-source tooling first.
